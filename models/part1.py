...
class SimpleCNN(nn.Module):
    def __init__(self):
       ...
       # ...
        ...
        self.fc1 = nn.Linear(128 * 4 * 4, 512)  # The input size here depends on the output of conv layers
    ...
    def forward(self, x):
        ...
        # Print the shape before flattening to see the feature map size
        print(f"x shape before flattening: {x.shape}")  # Add this line to check the shape

        # Flatten the tensor before feeding it into fully connected layers
        x = x.view(-1, 128 * 4 * 4)  # Adjust this based on the actual feature map size  # Flatten the output from conv layers
        ...
        # Output layer (no activation function here since we'll apply softmax during loss computation)
        x = self.fc2(x)
        return x
...
def train(epoch, model, trainloader, optimizer, criterion, CONFIG):
    ...
    for i, (inputs, labels) in enumerate(progress_bar):
        ...
        # Ensure labels are in the correct shape (flatten them if necessary)
        labels = labels.view(-1)

        # Ensure labels are of type long (for cross_entropy)
        labels = labels.long()
...
def main():
    ...
    transform_test = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),  # Same normalization as the training set
    ])   ### TODO -- BEGIN SOLUTION
